...
trait SortByTrait
{
    ...
    public function sortBy(array ...$sortInstructions): string
    {
        ...
        if (empty($sortInstructions)) {
            throw new SqlServicesException('Empty sortInstructions asked, modify the query to continue.', 5002005);
        }

        // Loop through the sorting instructions.
        foreach ($sortInstructions as $sortInstruction) {
            if (empty($sortInstruction)) {
                throw new SqlServicesException('Empty sortInstruction asked, modify the query to continue.', 5002006);
            } else {
                ...
                if (empty($columnName) || empty($sortOrder)) {
                    throw new SqlServicesException('Empty columnName or sortOrder asked, modify the query to continue.', 5002007);
                } elseif (!in_array($sortOrder, ['ASC', 'DESC'])) {
                    throw new SqlServicesException('Invalid sortOrder asked, modify the query to continue.', 5002008);
                } else {
